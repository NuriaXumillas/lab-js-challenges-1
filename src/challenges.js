...
function createSequence(n) {
  ...
  // si es 0 devolvemos el array vacío

  if (n === 0) {
    return sequence;
  }
  // si no, generamos de 0 a n
...
function multiplyBy(numbersArray, multiplier) {
  ...
  // si está vacío, devolvemos el array vacío
  if (numbersArray.length === 0) {
    return result;
  }

  // si no, mmultiplicamos el por el multiplier
 ...
 console.log(original, toRemove)
  for (let i = 0; i < original.length; i++) {
    ...
    for (let j = 0; j < toRemove.length; j++) {
      const removeElement = toRemove[j]

      if (originalElement === removeElement) {
...
function uniquifyArray(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    if(!arrWithoutReps.includes(array[i])){
      arrWithoutReps.push(array[i])
    }
  }

  if(array == arrWithoutReps){
    return arrWithoutReps
  }
